...
export default function BookingCalendar({ shopId }: { shopId: string }) {
  ...
  // --- 狀態管理 ---
  const [viewMode, setViewMode] = useState<'TODAY' | 'TOMORROW' | 'DAY_AFTER' | 'WEEK' | 'MONTH' | 'CUSTOM'>('TODAY');
  ...
  const handleViewModeChange = (mode: typeof viewMode) => {
    ...
    if (mode === 'TODAY') {
      ...
      const first = new Date(today); first.setDate(today.getDate() - today.getDay() + (today.getDay() === 0 ? -6 : 1)); // 週一為每週第一天
  ...
  // ========================================================
  // 視圖一：單日時間軸 (Gantt Chart Timeline) 演算法
  // ========================================================
  const isSingleDay = dateRange.start === dateRange.end;
  
  // 定義時間軸比例：1分鐘 = 1.5px (1小時 = 90px)，總長 24*90 = 2160px
  const PIXELS_PER_MIN = 1.5;
  const TIMELINE_WIDTH = 24 * 60 * PIXELS_PER_MIN;

  const getTimelineStyle = (isoStart: string, isoEnd: string) => {
    const start = new Date(isoStart); const end = new Date(isoEnd);
    const startMins = start.getHours() * 60 + start.getMinutes();
    const durationMins = (end.getTime() - start.getTime()) / 60000;
    return { left: `${startMins * PIXELS_PER_MIN}px`, width: `${durationMins * PIXELS_PER_MIN}px` };
  };

  // 生成時間標頭 (00:00 ~ 23:00)
  const timeHeaders = Array.from({ length: 24 }).map((_, i) => `${String(i).padStart(2, '0')}:00`);

  // ========================================================
  // 視圖二：多日月曆/週曆 (Calendar Grid) 演算法
  // ========================================================
  const calendarDays = useMemo(() => {
    ...
    // 若為月視圖，補齊前面的空白天數 (讓 1 號對齊星期幾)
    if (viewMode === 'MONTH') {
      const firstDayOfWeek = curr.getDay() === 0 ? 6 : curr.getDay() - 1; // 轉為週一開始 (0-6)
    ...
    while (curr <= end) {
      ...
      days.push({ date: dateStr, dayNum: curr.getDate(), bookings: dayBookings });
      curr.setDate(curr.getDate() + 1);
    }
    return days;
  }, [dateRange, bookings, isSingleDay, viewMode]);

  return (
    <div className="space-y-4 animate-fade-in w-full max-w-full overflow-hidden">
      
      {/* 頂部快捷控制列 */}
      <div className="bg-white p-3 md:p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex flex-col gap-3 w-full md:w-auto">
          {/* 日期快捷鍵 (加入明日、後天) */}
          <div className="flex gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100 overflow-x-auto no-scrollbar w-full md:w-auto">
            {[
              { id: 'TODAY', label: '今日' }, { id: 'TOMORROW', label: '明日' }, { id: 'DAY_AFTER', label: '後天' },
              { id: 'WEEK', label: '本週' }, { id: 'MONTH', label: '整月' }, { id: 'CUSTOM', label: '自訂' }
            ].map(m => (
              <button 
          ...
          {viewMode === 'CUSTOM' && (
            <div className="flex items-center gap-2">
              <input type="date" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} className="border border-slate-200 rounded px-2 py-1 text-sm outline-none" />
              <span className="text-slate-400">-</span>
              <input type="date" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} className="border border-slate-200 rounded px-2 py-1 text-sm outline-none" />
            </div>
          )}
        </div>

        <button onClick={() => setShowAddModal(true)} className="w-full md:w-auto px-5 py-2.5 bg-[#EE4D2D] text-white rounded-xl font-bold shadow-md hover:bg-[#d73211] transition flex items-center justify-center gap-2 text-sm">
      ...
      {isLoading ? (
        ...
        /* =========================================================
           [單日模式] - 24小時制員工時間軸 (Timeline / Gantt Chart)
           ========================================================= */
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h3 className="font-black text-lg text-slate-800"><i className="fa-solid fa-calendar-day text-purple-600 mr-2"></i>{dateRange.start} 預約排程</h3>
            <span className="text-sm font-bold text-slate-500">共 {bookings.length} 組</span>
          </div>
          
          <div className="flex w-full overflow-x-auto relative custom-scrollbar" ref={timelineRef}>
            {/* 左側：員工名單 (Sticky 凍結) */}
            <div className="w-24 md:w-32 shrink-0 bg-white border-r border-slate-200 sticky left-0 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.05)] flex flex-col">
              <div className="h-10 border-b border-slate-200 bg-slate-50"></div> {/* 對齊時間標頭 */}
              
              {/* ★ 移除：依要求不再顯示不指定員工列，預約將全數自動派發給實體員工 */}
              
              {/* 各員工列 */}
              {staffList.map(staff => (
                <div key={staff.id} className="h-24 md:h-20 border-b border-slate-100 flex flex-col items-center justify-center p-1 md:p-2 bg-white">
                   <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-100 border border-slate-200 overflow-hidden mb-1">
                      <img src={staff.avatar_url || `https://ui-avatars.com/api/?name=${staff.nickname || staff.name}`} className="w-full h-full object-cover" />
                   </div>
                   <div className="text-[10px] md:text-xs font-bold text-slate-700 text-center break-all line-clamp-1">{staff.nickname || staff.name}</div>
                </div>
              ))}
            </div>

            {/* 右側：24小時時間軸網格 */}
            <div className="relative" style={{ width: `${TIMELINE_WIDTH}px`, minWidth: `${TIMELINE_WIDTH}px` }}>
              {/* 時間標頭列 */}
              <div className="h-10 flex border-b border-slate-200 bg-slate-50 absolute top-0 left-0 right-0 z-10">
                {timeHeaders.map((time, i) => (
                  <div key={time} className="border-r border-slate-200 text-[10px] text-slate-400 font-bold px-1 pt-1" style={{ width: `${60 * PIXELS_PER_MIN}px` }}>
                    {time}
                  </div>
                ))}
              </div>
              ...
              {/* 網格背景與預約區塊層 */}
              <div className="mt-10 flex flex-col relative z-0">
                {/* 背景格線 */}
                <div className="absolute inset-0 flex pointer-events-none z-0 opacity-30">
                   {timeHeaders.map((_, i) => (
                     <div key={i} className="border-r border-slate-200 h-full" style={{ width: `${60 * PIXELS_PER_MIN}px` }}></div>
                   ))}
                </div>

                {/* ★ 移除：依要求不再顯示不指定員工的預約橫列 (系統已實作自動派單) */}

                {/* 2. 個別員工的橫列 (包含休息時間灰底) */}
                {staffList.map(staff => {
                  const dayOfWeek = new Date(dateRange.start).getDay();
                  const schedule = staff.work_schedule?.[String(dayOfWeek)] || { active: true, start: storeSetting?.default_open_time || '10:00', end: storeSetting?.default_close_time || '21:00' };
                  const isClosed = storeSetting?.closed_dates?.includes(dateRange.start);
                  
                  // 計算灰底區域
                  let grayAreas = [];
                  if (isClosed || !schedule.active) {
                    grayAreas.push({ left: 0, width: TIMELINE_WIDTH }); // 全天休息
                  } else {
                    const startMins = parseInt(schedule.start.split(':')[0]) * 60 + parseInt(schedule.start.split(':')[1]);
                    const endMins = parseInt(schedule.end.split(':')[0]) * 60 + parseInt(schedule.end.split(':')[1]);
                    if (startMins > 0) grayAreas.push({ left: 0, width: startMins * PIXELS_PER_MIN }); // 上班前
                    if (endMins < 1440) grayAreas.push({ left: endMins * PIXELS_PER_MIN, width: (1440 - endMins) * PIXELS_PER_MIN }); // 下班後
                  }

                  return (
                    <div key={staff.id} className="h-24 md:h-20 border-b border-slate-100 relative w-full">
                      {/* 休息時間遮罩 (灰色斜線背景) */}
                      {grayAreas.map((area, i) => (
                        <div 
                          key={i} 
                          className="absolute top-0 bottom-0 bg-slate-100/60 flex items-center justify-center z-0" 
                          style={{ 
                            left: `${area.left}px`, 
                            width: `${area.width}px`,
                        ...
                        >
                          {area.width > 100 && <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">OFF DUTY</span>}
                        </div>
                      ))}

                      {/* 該員工的預約區塊 */}
                      {bookings.filter(b => b.staff_id === staff.id).map(b => (
                         <div key={b.id} style={getTimelineStyle(b.start_time, b.end_time)} className="absolute top-1 bottom-1 p-1 md:p-1.5 z-10">
                           <div onClick={() => setSelectedBooking(b)} className={`w-full h-full rounded-lg border shadow-sm p-1.5 flex flex-col justify-center overflow-hidden transition cursor-pointer hover:-translate-y-0.5 hover:shadow-md ${b.status === 'COMPLETED' ? 'bg-green-50 border-green-200 text-green-700' : b.status === 'PENDING' ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-purple-50 border-purple-200 text-purple-700'}`}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="grid grid-cols-7 bg-white">
            {calendarDays.map((dayObj, idx) => {
              if (!dayObj) return <div key={idx} className="min-h-[100px] border-b border-r border-slate-100 bg-slate-50/50"></div>; // 空白墊片
              
              const isToday = dayObj.date === getLocalDateString(new Date());
              const isClosed = storeSetting?.closed_dates?.includes(dayObj.date);

              return (
                <div key={idx} className={`min-h-[120px] border-b border-r border-slate-100 p-1 md:p-2 flex flex-col transition hover:bg-slate-50 ${isClosed ? 'bg-slate-100/50' : ''}`}>
                  ...
                  {isClosed ? (
                    ...
                    <>
                      <div className="text-[10px] text-slate-400 mb-1 text-center border-b border-slate-100 pb-1">合計 <span className="font-black text-slate-700 text-xs">{dayObj.bookings.length}</span></div>
                      <div className="flex flex-col gap-1 overflow-y-auto custom-scrollbar flex-1 max-h-[80px]">
                        {dayObj.bookings.slice(0, 4).map(b => (
                          <div key={b.id} onClick={() => setSelectedBooking(b)} className={`cursor-pointer hover:opacity-80 transition text-[9px] md:text-[10px] px-1 py-0.5 rounded font-bold truncate ${b.status === 'COMPLETED' ? 'bg-green-50 text-green-600' : b.status === 'PENDING' ? 'bg-orange-50 text-orange-600' : 'bg-purple-50 text-purple-600'}`}>
